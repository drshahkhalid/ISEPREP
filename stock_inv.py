...
def get_active_designation(code):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("""SELECT designation, designation_en, designation_fr, designation_sp
                          FROM items_list WHERE code = ?""", (code,))
        row = cursor.fetchone()
        if not row:
            return code
...
def get_std_qty(code, scenario_name):
    ...
    try:
        if scenario_name == lang.t("stock_inv.all_scenarios", "All Scenarios"):
            cursor.execute("SELECT SUM(quantity) AS total_qty FROM compositions WHERE code = ?", (code,))
            row = cursor.fetchone()
...
def get_item_type(code: str) -> str:
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT type, designation_en FROM items_list WHERE code = ?", (code,))
        row = cur.fetchone()
        if row and row['type']:
...
def parse_inventory_unique_id(unique_id: str) -> dict:
    ...
    out = {
        ...
        "kit_code": None, "module_code": None, "item_code": None,
        "std_qty": None, "exp_date": None,
...
def construct_unique_id(scenario_id: str,
                        kit_code: str,
                        module_code: str,
                        item_code: str,
                        std_qty: int,
                        ...
                        force_box_format: bool = False) -> str:
    base = [
        scenario_id or "0",
        kit_code or "None",
        module_code or "None",
...
class StockInventory(tk.Frame):
        ...
        try:
            sql = """SELECT DISTINCT kit_number
                      ...
                      WHERE kit_number IS NOT NULL
                        AND kit_number != 'None'"""
            params = []
            if scenario_name and scenario_name != lang.t("stock_inv.all_scenarios", "All Scenarios"):
                sql += " AND scenario = ?"
    ...
    def fetch_search_results(self, query):
        ...
        try:
            ...
            if self.scenario_var.get() != lang.t("stock_inv.all_scenarios", "All Scenarios"):
    ...
    def fetch_items_in_stock(self, code=None):
        ...
        cur = conn.cursor()
        try:
            sql = """SELECT unique_id, qty_in, qty_out FROM stock_data WHERE (qty_in - qty_out) != 0"""
            params = []
            if code:
            ...
            for r in rows:
                ...
                scenario_name = parsed["scenario_name"] or lang.t("stock_inv.unknown", "Unknown")

                if scenario_filter != lang.t("stock_inv.all_scenarios", "All Scenarios") and scenario_name != scenario_filter:
                    continue
                ...
                display_code = parsed["item_code"] or parsed["module_code"] or parsed["kit_code"] or ""
                description = get_active_designation(display_code)
                item_type = get_item_type(display_code)
                current_stock = (r['qty_in'] or 0) - (r['qty_out'] or 0)
    ...
    def capture_current_rows(self):
        for iid in self.tree.get_children():
            ...
            if not vals:
                continue
            uid = vals[0]
            base_ph = self.base_physical_inputs.get(iid, int(vals[9]) if str(vals[9]).isdigit() else 0)
    ...
    def build_filtered_dataset(self):
        ...
        for item in fetched:
            ...
            if state:
                ...
                base_ph = state.get("base_physical", int(phys) if str(phys).isdigit() else 0)
            else:
        ...
        for uid, state in self.user_row_states.items():
            if uid not in fetched_map and state.get("is_custom"):
                ...
                if scen_filter != lang.t("stock_inv.all_scenarios", "All Scenarios") and state["scenario"] != scen_filter:
                    continue
                merged.append((
                    {
    ...
    def rebuild_tree_preserving_state(self):
        ...
        is_complete_inventory = self.inv_type_var.get() == lang.t("stock_inv.complete_inventory", "Complete Inventory")
    ...
    def _insert_from_state(self, item_dict, physical_qty, updated_exp, discrepancy, remarks, base_physical):
    ...
    def generate_document_number(self):
        ...
        abbr = "CINV" if inv_type_label == lang.t("stock_inv.complete_inventory", "Complete Inventory") else "PINV"
        now = datetime.now()
    ...
    def export_to_excel(self, rows_to_export=None, document_number=None):
        try:
            ...
            if not path:
                self.status_var.set(lang.t("stock_inv.export_cancelled", "Export cancelled"))
                return
            ...
            ws['A4'] = f"{lang.t('stock_inv.inventory_date', 'Inventory Date')}: {current_date}"
            ws['A4'].font = Font(name="Tahoma")
            ws['A4'].alignment = Alignment(horizontal="right")
            ws.merge_cells('A4:L4')

            if document_number:
                ws['A5'] = f"{lang.t('stock_inv.document_number', 'Document Number')}: {document_number}"
            ...
            ws.append(headers)
            kit_fill = PatternFill(start_color="D8F5D0", end_color="D8F5D0", fill_type="solid")
            module_fill = PatternFill(start_color="D5ECFF", end_color="D5ECFF", fill_type="solid")
            exp_warn_fill = PatternFill(start_color="FFD8D8", end_color="FFD8D8", fill_type="solid")
            ...
            for idx, row in enumerate(rows_data, start=row_start):
                ws.append([
                    row['code'], row['description'], row['type'], row['scenario'],
                    row['kit_number'], row['module_number'], row['current_stock'],
                    row['exp_date'], row['physical_qty'], row['updated_exp_date'],
                    row['discrepancy'], row['remarks']
                ])
                t = (row['type'] or "").lower()
                fill = None
                if t == "kit":
                ...
                if check_expiry_required(row['code']) and not (row['updated_exp_date'] or row['exp_date']):
                    fill = exp_warn_fill
                if fill:
            ...
            custom_popup(self, lang.t("dialog_titles.success", "Success"),
                         lang.t("stock_inv.exported", "Exported to {path}").format(path=path), "info")
            self.status_var.set(lang.t("stock_inv.export_success", "Export successful: {path}").format(path=path))
        except Exception as e:
            custom_popup(self, lang.t("dialog_titles.error", "Error"),
                         lang.t("stock_inv.export_failed", "Export failed: {error}").format(error=str(e)),
                         "error")
            self.status_var.set(lang.t("stock_inv.export_error", "Export error: {error}").format(error=str(e)))
    ...
    def render_ui(self):
        ...
        tk.Label(self,
                 ...
                 bg="#F5F5F5").pack(pady=10, anchor="w")

        filter_frame = tk.Frame(self, bg="#F5F5F5")
        filter_frame.pack(pady=5, fill="x")

        tk.Label(filter_frame, text=lang.t("stock_inv.scenario", "Scenario:"), bg="#F5F5F5")\
            .grid(row=0, column=0, padx=5, sticky="w")
        self.scenario_var = tk.StringVar(value=lang.t("stock_inv.all_scenarios", "All Scenarios"))
        ...
        self.scenario_cb = ttk.Combobox(filter_frame, textvariable=self.scenario_var,
                                        values=scenarios, state="readonly", width=20)
        self.scenario_cb.grid(row=0, column=1, padx=5, pady=5)
        self.scenario_cb.bind("<<ComboboxSelected>>", self.on_scenario_selected)

        tk.Label(filter_frame, text=lang.t("stock_inv.management_mode", "Management Mode:"), bg="#F5F5F5")\
            .grid(row=0, column=2, padx=5, sticky="w")
        self.mgmt_mode_var = tk.StringVar(value=lang.t("stock_inv.management_all", "All"))
        ...
        tk.Label(filter_frame, text=lang.t("stock_inv.kit_number", "Kit Number:"), bg="#F5F5F5")\
            .grid(row=0, column=4, padx=5, sticky="w")
        self.kit_number_var = tk.StringVar(value=lang.t("stock_inv.all_kits", "All Kits"))
        self.kit_number_cb = ttk.Combobox(filter_frame, textvariable=self.kit_number_var,
        ...
        tk.Label(filter_frame, text=lang.t("stock_inv.module_number", "Module Number:"), bg="#F5F5F5")\
            .grid(row=0, column=6, padx=5, sticky="w")
        self.module_number_var = tk.StringVar(value=lang.t("stock_inv.all_modules", "All Modules"))
        self.module_number_cb = ttk.Combobox(filter_frame, textvariable=self.module_number_var,
        ...
        tk.Label(type_frame, text=lang.t("stock_inv.inventory_type", "Inventory Type:"), bg="#F5F5F5")\
            .grid(row=0, column=0, padx=5, sticky="w")
        self.inv_type_var = tk.StringVar(value=lang.t("stock_inv.complete_inventory", "Complete Inventory"))
        self.inv_type_cb = ttk.Combobox(type_frame, textvariable=self.inv_type_var, state="readonly",
                                        values=[
        ...
        tk.Label(self.search_frame, text=lang.t("stock_inv.search", "Search Code or Description:"), bg="#F5F5F5")\
            .grid(row=0, column=0, padx=5, sticky="w")
        ...
        tk.Button(self.button_frame, text=lang.t("stock_inv.add_missing_item", "Add Missing Item"),
                  bg="#2980B9", fg="white", command=self.add_missing_item).pack(side="right", padx=5, pady=5)

        # Tree
        tree_frame = tk.Frame(self)
        tree_frame.pack(expand=True, fill="both", pady=10)
        self.cols = (
            "unique_id", "code", "description", "type", "scenario",
            "kit_number", "module_number", "current_stock", "exp_date",
            "physical_qty", "updated_exp_date", "discrepancy", "remarks", "std_qty"
        )
        self.display_cols = (
            "code", "description", "type", "scenario",
            "kit_number", "module_number", "current_stock",
            "exp_date", "physical_qty", "updated_exp_date", "discrepancy", "remarks"
        )
        self.tree = ttk.Treeview(tree_frame, columns=self.cols, show="headings",
                                 height=18, displaycolumns=self.display_cols)
        ...
        headers = {
            ...
            "updated_exp_date": lang.t("stock_inv.updated_exp_date", "Updated Expiry Date"),
        ...
        actions = [
            ("Save Adjustments", "#27AE60", self.save_all, "normal" if self.role in ["admin", "manager"] else "disabled"),
            ("Clear All", "#7F8C8D", self.clear_form, "normal"),
            ("Export to Excel", "#3B82F6", self.export_to_excel, "normal" if self.role in ["admin", "manager"] else "disabled")
        ]
        for txt, bg, cmd, st in actions:
            tk.Button(btn_frame, text=lang.t(f"stock_inv.{txt.lower().replace(' ', '_')}", txt),
                      bg=bg, fg="white", command=cmd, state=st).pack(side="left", padx=5)

        batch_frame = tk.Frame(self, bg="#F5F5F5")
        batch_frame.pack(pady=5, fill="x")
        self.batch_info_var = tk.StringVar()
        tk.Label(batch_frame, text=lang.t("stock_inv.batch_info", "Batch Information:"),
                 bg="#F5F5F5").grid(row=0, column=0, padx=5, sticky="w")
        tk.Label(batch_frame, textvariable=self.batch_info_var, bg="#F5F5F5",
                 wraplength=600, anchor="w").grid(row=0, column=1, padx=5, sticky="w")

        self.status_var = tk.StringVar(value="")
        tk.Label(self, textvariable=self.status_var, bg="#F5F5F5",
                 anchor="w").pack(pady=5, anchor="w")
    ...
    def _update_state_from_row(self, row_id):
        ...
        st["remarks"] = vals[12]
        st["base_physical"] = self.base_physical_inputs.get(row_id, st.get("base_physical", 0))
    ...
    def add_new_row_below(self, row_id):
        ...
        self._highlight_missing_required_expiry()
        self.status_var.set(
            lang.t("stock_inv.added_new_row", "Added new row below {code}").format(code=vals[1])
        )
    ...
    def on_filter_change(self, event=None):
        ...
        if not on_shelf and widget == getattr(self, "kit_number_cb", None):
            self.refresh_module_dropdown()
        if self.inv_type_var.get() == lang.t("stock_inv.complete_inventory", "Complete Inventory"):
            self.rebuild_tree_preserving_state()
    ...
    def on_inv_type_selected(self, event=None):
        ...
        if inv_type == lang.t("stock_inv.complete_inventory", "Complete Inventory"):
            ...
            self.batch_info_var.set(lang.t("stock_inv.partial_batch_info",
    ...
    def on_search_keyrelease(self, event=None):
        ...
        self.status_var.set(
            lang.t("stock_inv.found_items",
                   "Found {count} items matching '{query}'").format(count=len(results), query=query)
        )
    ...
    def on_search_select(self, event=None):
        ...
        for rec in items:
            if rec['unique_id'] not in self.user_row_states:
                ...
                added += 1
        self._highlight_missing_required_expiry()
        self.status_var.set(lang.t("stock_inv.added_items",
                                   "Added items for code {code}").format(code=code))
        self.batch_info_var.set(lang.t("stock_inv.added_lines",
    ...
    def add_missing_item(self):
        ...
        scen_cb_var = tk.StringVar(value=self.scenario_var.get()
                                   if self.scenario_var.get() != lang.t("stock_inv.all_scenarios", "All Scenarios")
                                   else (scenario_values[0] if scenario_values else ""))
        scen_cb = ttk.Combobox(dialog, textvariable=scen_cb_var,
                               values=scenario_values, state="readonly", width=30)
        scen_cb.pack(pady=2)
        ...
        tk.Label(dialog, text=lang.t("stock_inv.module_code", "Module Code:")).pack(pady=4)
        module_code_var = tk.StringVar()
        tk.Entry(dialog, textvariable=module_code_var, width=30).pack()

        tk.Label(dialog, text=lang.t("stock_inv.code", "Item Code - Description:")).pack(pady=4)
        ...
        tk.Label(dialog, text=lang.t("stock_inv.std_qty", "Standard Quantity:")).pack(pady=4)
        std_qty_var = tk.StringVar(value="0")
        tk.Label(dialog, textvariable=std_qty_var, width=30).pack()

        tk.Label(dialog, text=lang.t("stock_inv.physical_qty", "Physical Quantity:")).pack(pady=4)
        physical_qty_var = tk.StringVar(value="")  # blank default
        tk.Entry(dialog, textvariable=physical_qty_var, width=30).pack()

        tk.Label(dialog, text=lang.t("stock_inv.expiry_date", "Expiry (MM/YYYY):")).pack(pady=4)
        exp_date_var = tk.StringVar()
        tk.Entry(dialog, textvariable=exp_date_var, width=30).pack()

        tk.Label(dialog, text=lang.t("stock_inv.kit_number", "Kit Number:")).pack(pady=4)
        kit_number_var = tk.StringVar(value=self.kit_number_var.get()
                                      if self.kit_number_var.get() != lang.t("stock_inv.all_kits", "All Kits") else "")
        tk.Entry(dialog, textvariable=kit_number_var, width=30).pack()

        tk.Label(dialog, text=lang.t("stock_inv.module_number", "Module Number:")).pack(pady=4)
        module_number_var = tk.StringVar(value=self.module_number_var.get()
                                         if self.module_number_var.get() != lang.t("stock_inv.all_modules", "All Modules") else "")
        ...
        def update_code_list(event=None):
            ...
            code_only = code_var.get().split(" - ")[0] if " - " in code_var.get() else code_var.get()
            if code_only:
        ...
        def submit_new():
            ...
            if not scenario_name:
                custom_popup(self, lang.t("dialog_titles.error", "Error"),
                             lang.t("stock_inv.no_scenario", "Please select a scenario."), "error")
                return
            scenario_id = None
            ...
            if not scenario_id:
                custom_popup(self, lang.t("dialog_titles.error", "Error"),
                             lang.t("stock_inv.no_scenario", "Scenario not found."), "error")
                return
            item_code = code_var.get().strip()
            if not item_code:
                custom_popup(self, lang.t("dialog_titles.error", "Error"),
                             lang.t("stock_inv.no_code", "Please select a code."), "error")
                return
            physical = physical_qty_var.get().strip()
            if physical and (not physical.isdigit() or int(physical) < 0):
                custom_popup(self, lang.t("dialog_titles.error", "Error"),
                             lang.t("stock_inv.invalid_qty", "Physical Quantity must be a non-negative integer."), "error")
                return
            ...
            if check_expiry_required(item_code):
                if not validate_expiry_for_save(item_code, exp_raw):
                    custom_popup(self, lang.t("dialog_titles.error", "Error"),
                                 lang.t("stock_inv.invalid_expiry", "A valid future expiry date is required."), "error")
                    return
            ...
            self.insert_tree_row(record, physical_qty=(physical if physical else ""))
            self._highlight_missing_required_expiry()
            self.batch_info_var.set(lang.t("stock_inv.added_new_item_info",
                                           "Added new item {code} with physical quantity {qty}")
                                    .format(code=item_code, qty=physical if physical else ""))
            self.status_var.set(lang.t("stock_inv.added_new_item",
                                       "Added new item {code}").format(code=item_code))
            dialog.destroy()

        tk.Button(dialog, text=lang.t("stock_inv.submit", "Submit"),
                  bg="#27AE60", fg="white", command=submit_new).pack(pady=10)
    ...
    def save_all(self):
        """
        Save adjustments with RELIABLE expiry-change handling.

        Expiry change logic:
          - Detect by: updated_exp_date (col 10) != exp_date (col 8) and updated_exp_date not blank.
          - Use ORIGINAL unique_id (still pointing to old expiry) to fetch old batch.
          - old_final = (qty_in - qty_out + discrepancy_if_present).
          - Zero old batch: qty_out = qty_in, discrepancy = 0 => final becomes 0.
          - Log single OUT transaction for old_final (if old_final > 0).
          - Insert new batch with new unique_id (constructed using the new expiry) and:
              qty_in = physical_qty (if provided; if blank -> old_final to fully migrate).
              qty_out = 0, discrepancy = 0.
          - Log single IN transaction for the inserted quantity (if > 0).
          - If physical differs from old_final, that difference is implicitly a variance
            (no discrepancy needed; variance reflected in total net movement).
        Expiry unchanged:
          - If physical blank -> no change.
          - Else delta = physical - current_final
                >0: qty_in += delta
                <0: qty_out += -delta
            Always reset discrepancy=0 if column exists.
            Log IN/OUT delta transaction.
        New row (not in stock_data):
          - Insert only if physical > 0 (same rules as above; if the user set updated_exp_date
            it is treated as the expiry for the new line).

        Discrepancy column:
          - If present, always forcibly reset to 0 after operations (we rely on qty_in / qty_out only).

        Validation:
          - If item requires expiry and physical > 0 -> updated_exp_date must be valid & future (else block).
        """

        if self.role.lower() not in ["admin", "manager"]:
            custom_popup(self, lang.t("dialog_titles.error", "Error"),
                         lang.t("stock_inv.restricted", "Only admin or manager roles can save changes."), "error")
            return

        rows = self.tree.get_children()
        if not rows:
            custom_popup(self, lang.t("dialog_titles.error", "Error"),
                         lang.t("stock_inv.no_rows", "No rows to save."), "error")
            return
        ...
        if blocking:
            self._highlight_missing_required_expiry()
            custom_popup(self, lang.t("dialog_titles.error", "Error"),
                         lang.t("stock_inv.invalid_expiry",
                                "Valid future updated expiry dates are required for items: {items}")
                         .format(items=", ".join(sorted(set(blocking)))), "error")
            return
        ...
        if conn is None:
            custom_popup(self, lang.t("dialog_titles.error", "Error"),
                         lang.t("stock_inv.db_error", "Database connection failed"), "error")
            return
        cur = conn.cursor()
        ...
        # Detect discrepancy column
        cur.execute("PRAGMA table_info(stock_data)")
        colset = {r[1].lower() for r in cur.fetchall()}
        has_discrepancy = 'discrepancy' in colset

        inv_type_label = self.inv_type_var.get()
        inv_abbr = "Complete INV" if inv_type_label == lang.t("stock_inv.complete_inventory", "Complete Inventory") else "Partial INV"
        ...
        def log(uid, exp, qty_in=None, qty_out=None, remarks=None, code=None, scen=None, kit=None, mod=None):
        ...
        try:
            ...
            for rid in rows:
                vals = self.tree.item(rid, "values")
                (unique_id, code, _desc, _type, scenario_name, kit_number, module_number,
                 _curr_stock_ui, exp_date_orig, phys_str, updated_exp, _disc_ui, remarks, std_qty_ui) = vals
                ...
                std_qty_val = parsed["std_qty"] if parsed["std_qty"] is not None else (
                    int(std_qty_ui) if str(std_qty_ui).isdigit() else 0
                )

                # Fetch existing (original unique_id)
                if has_discrepancy:
                    cur.execute("SELECT qty_in, qty_out, discrepancy FROM stock_data WHERE unique_id = ?", (unique_id,))
                else:
                    cur.execute("SELECT qty_in, qty_out FROM stock_data WHERE unique_id = ?", (unique_id,))
                existing = cur.fetchone()

                # Helper to insert new batch
                def insert_new_batch(new_uid, exp_val, qty_in_amount):
                    cols = ["unique_id", "scenario", "kit_number", "module_number",
                            "kit", "module", "item", "std_qty", "qty_in", "qty_out"]
                    vals_ = [new_uid, scenario_name, parsed["kit_number"], parsed["module_number"],
                             kit_code, module_code, item_code, std_qty_val, qty_in_amount, 0]
                    if has_discrepancy:
                        cols.append("discrepancy")
                        vals_.append(0)
                    cols.append("exp_date")
                    vals_.append(exp_val)
                    attempt(f"INSERT INTO stock_data ({', '.join(cols)}) VALUES ({', '.join(['?']*len(cols))})", tuple(vals_))

                if not existing:
                    # New row
                    if physical and physical > 0:
                        target_exp = new_exp if expiry_changed else old_exp
                        had_box = parsed["kit_number"] or parsed["module_number"] or len(unique_id.split("/")) >= 8
                        ...
                        log(new_uid, target_exp, qty_in=physical, code=code,
                            scen=scenario_name, kit=kit_number, mod=module_number, remarks=remarks)
                    continue

                old_final = final_qty(existing)

                if expiry_changed:
                    # Zero old batch (always), log OUT of old_final if >0
                    if old_final != 0:
                        # qty_out = qty_in, discrepancy=0
                        if has_discrepancy:
                            attempt("UPDATE stock_data SET qty_out = qty_in, discrepancy = 0 WHERE unique_id = ?", (unique_id,))
                        else:
                            attempt("UPDATE stock_data SET qty_out = qty_in WHERE unique_id = ?", (unique_id,))
                        if old_final > 0:
                            log(unique_id, old_exp, qty_out=old_final, code=code,
                                scen=scenario_name, kit=kit_number, mod=module_number, remarks=remarks)
                        elif old_final < 0:
                            # Negative inventory correction (rare)
                            log(unique_id, old_exp, qty_in=-old_final, code=code,
                                scen=scenario_name, kit=kit_number, mod=module_number, remarks=remarks)

                    # Determine new batch qty_in:
                    new_qty_in = physical if (physical is not None) else old_final
                    if new_qty_in and new_qty_in > 0:
                        had_box = parsed["kit_number"] or parsed["module_number"] or len(unique_id.split("/")) >= 8
                        new_uid = construct_unique_id(
                            scenario_id=scenario_id,
                            kit_code=kit_code,
                        ...
                        insert_new_batch(new_uid, new_exp, new_qty_in)
                        log(new_uid, new_exp, qty_in=new_qty_in, code=code,
                            scen=scenario_name, kit=kit_number, mod=module_number, remarks=remarks)
                    continue
                ...
                if physical is None:
                    continue
                delta = physical - old_final
                if delta == 0:
                    # Just normalize discrepancy if present
                    if has_discrepancy and (existing[-1] if has_discrepancy else 0) != 0:
                        attempt("UPDATE stock_data SET discrepancy = 0 WHERE unique_id = ?", (unique_id,))
                    continue
                if delta > 0:
                    if has_discrepancy:
                        attempt("UPDATE stock_data SET qty_in = qty_in + ?, discrepancy = 0 WHERE unique_id = ?",
                                (delta, unique_id))
                    else:
                        attempt("UPDATE stock_data SET qty_in = qty_in + ? WHERE unique_id = ?",
                                (delta, unique_id))
                    log(unique_id, old_exp, qty_in=delta, code=code,
                        scen=scenario_name, kit=kit_number, mod=module_number, remarks=remarks)
                else:
                    out_qty = -delta
                    if has_discrepancy:
                        attempt("UPDATE stock_data SET qty_out = qty_out + ?, discrepancy = 0 WHERE unique_id = ?",
                                (out_qty, unique_id))
                    else:
                        attempt("UPDATE stock_data SET qty_out = qty_out + ? WHERE unique_id = ?",
                                (out_qty, unique_id))
                    log(unique_id, old_exp, qty_out=out_qty, code=code,
                        scen=scenario_name, kit=kit_number, mod=module_number, remarks=remarks)
            ...
            if errors:
                ...
                custom_popup(self, lang.t("dialog_titles.success", "Success"),
                             lang.t("stock_inv.success", "Inventory adjustments saved successfully."), "info")
                self.user_row_states.clear()
                self.base_physical_inputs.clear()
            ...
            conn.rollback()
            custom_popup(self, lang.t("dialog_titles.error", "Error"),
                         lang.t("stock_inv.save_failed", "Failed to save: {error}").format(error=str(e)), "error")
        finally:
            cur.close()
            conn.close()
    ...
    def clear_form(self):
        ...
        self.code_entry.delete(0, tk.END)
        self.search_listbox.delete(0, tk.END)
        self.inv_type_var.set(lang.t("stock_inv.complete_inventory", "Complete Inventory"))
