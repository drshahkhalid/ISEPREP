...
class StockOutKit(tk.Frame):
    ...
    def __init__(self, parent, app, role="supervisor"):
        ...
        self.out_type_var = tk.StringVar(
            value=""
        )  # Will be set after lang is available
    ...
    def build_mode_definitions(self):
        ...
        self.mode_definitions = [
            ...
            (
                "out_standalone",
                lang.t(
                    "out_kit.mode_out_standalone",
                    "Out standalone item/s from {scenario}",
                    scenario=scenario,
                ),
            ),
            (
                "out_module_scenario",
                lang.t(
                    "out_kit.mode_out_module_scenario",
                    "Out module from {scenario}",
                    scenario=scenario,
                ),
            ),
            (
                "out_module_kit",
                lang.t("out_kit.mode_out_module_kit", "Out module from a Kit"),
            ),
            (
                "out_items_kit",
                lang.t("out_kit.mode_out_items_kit", "Out items from a Kit"),
            ),
            (
                "out_items_module",
                lang.t("out_kit.mode_out_items_module", "Out items from a module"),
            ),
        ]
    ...
    @staticmethod
    def generate_onshelf_unique_id(
        inbox_unique_id: str, item_type: str, exp_date: str = None
    ) -> str:
        """
        ...
        if len(parts) < 6:
            logging.warning(
                f"[GENERATE_ONSHELF] Invalid unique_id format: {inbox_unique_id}"
            )
            return inbox_unique_id
        ...
        logging.debug(
            f"[GENERATE_ONSHELF] {item_type}: {inbox_unique_id} → {onshelf_id}"
        )
        return onshelf_id
    ...
    def check_if_adopted_expiry(self, unique_id_inbox):
        ...
        try:
            ...
            logging.debug(
                f"[ADOPTED_CHECK] {unique_id_inbox}: adopted={is_adopted}, comment={row['comments'][:50] if row['comments'] else 'None'}"
            )
    ...
    def check_if_item_requires_expiry(self, item_code):
        ...
        try:
            ...
            logging.debug(
                f"[EXPIRY_CHECK] {item_code}: requires={requires_expiry}, remarks={remarks[:50] if remarks else 'None'}"
            )
    ...
    def _parse_flexible_date(self, date_input):
        ...
        patterns = [
            ...
            (r"^(\d{4})$", "YYYY", lambda m: f"{m[0]}-12-31"),
        ...
        for pattern, format_name, converter in patterns:
            ...
            if match:
                try:
                    ...
                    logging.debug(
                        f"[PARSE_DATE] '{date_input}' ({format_name}) → '{result}'"
                    )
                    return True, result, ""

                except (ValueError, IndexError) as e:
                    logging.debug(
                        f"[PARSE_DATE] Pattern {format_name} matched but failed validation: {e}"
                    )
                    continue
    ...
    def _parse_ambiguous_slash_date(self, groups):
        ...
        logging.debug(
            f"[PARSE_DATE] Ambiguous date {first}/{second}/{year}, assuming DD/MM/YYYY"
        )
        return f"{year}-{second:02d}-{first:02d}"
    ...
    def on_mode_changed(self, event=None):
        ...
        if mode_key == "out_kit":
            ...
            logging.debug(
                f"[OUT_KIT] out_kit: Populated {len(self.Kit_cb['values'])} primary kits"
            )
            ...
            logging.debug(
                f"[OUT_KIT] out_module_scenario: Populated {len(modules)} primary modules"
            )
            ...
            self.Kit_cb["values"] = self.fetch_Kits(self.selected_scenario_id)
            logging.debug(
                f"[OUT_KIT] out_module_kit: Populated {len(self.Kit_cb['values'])} primary kits"
            )
            ...
            self.Kit_cb["values"] = self.fetch_Kits(self.selected_scenario_id)
            logging.debug(
                f"[OUT_KIT] out_items_kit: Populated {len(self.Kit_cb['values'])} primary kits"
            )
    ...
    def ensure_item_index(self, scenario_id):
        if self._item_index_cache.get(
            "scenario_id"
        ) == scenario_id and self._item_index_cache.get("flat_map"):
            return
        self._item_index_cache = {
    ...
    def enrich_stock_row(
        ...
        entry = idx.get("triple_map", {}).get(triple_key) or idx.get(
            "flat_map", {}
        ).get(display_code or "")
        ...
        final_type = (
            forced
            if forced in ("Module", "Item")
            else ("Kit" if detected_norm not in ("Kit",) else detected_norm)
        )
    ...
    def fetch_stock_data_for_Kit_number(self, scenario_id, Kit_number, Kit_code=None):
        ...
        if conn is None:
            logging.error(
                "[OUT_KIT] DB connection failed in fetch_stock_data_for_Kit_number"
            )
            return []
        ...
        try:
            ...
            logging.info(
                f"[OUT_KIT] Found {len(result)} in-box items for Kit_number={Kit_number}"
            )
            return result
    ...
    def fetch_stock_data_for_module_number(
        ...
        if conn is None:
            logging.error(
                "[OUT_KIT] DB connection failed in fetch_stock_data_for_module_number"
            )
            return []
        ...
        try:
            ...
            logging.info(
                f"[OUT_KIT] Found {len(result)} in-box items for module_number={module_number}"
            )
            return result
    ...
    def fetch_standalone_stock_items(self, scenario_id):
        ...
        if conn is None:
            logging.error(
                "[OUT_KIT] DB connection failed in fetch_standalone_stock_items"
            )
            return []
    ...
    def fetch_Kits(self, scenario_id):
        ...
        try:
            ...
            if not kit_codes:
                logging.debug(
                    f"[OUT_KIT] No primary kits found for scenario {scenario_id}"
                )
                return []
            ...
            logging.info(
                f"[OUT_KIT] Found {len(result)} primary kits for scenario {scenario_id}"
            )
            return result
    ...
    def fetch_all_modules(self, scenario_id):
        ...
        try:
            ...
            if not module_codes:
                logging.debug(
                    f"[OUT_KIT] No primary modules found for scenario {scenario_id}"
                )
                return []
            ...
            logging.info(
                f"[OUT_KIT] Found {len(result)} primary modules for scenario {scenario_id}"
            )
            return result
    ...
    def fetch_available_Kit_numbers(self, scenario_id, Kit_code=None):
        ...
        if conn is None:
            logging.error(
                "[OUT_KIT] DB connection failed in fetch_available_Kit_numbers"
            )
            return []
    ...
    def render_ui(self):
        ...
        logging.info(
            "[OUT_KIT] UI rendered successfully matching dispatch_kit.py style"
        )
    ...
    def configure_tree_tags(self):
        ...
        try:
            ...
            # ===== Data row styles =====
            self.tree.tag_configure(
                "kit_data", background="#C5EDC1"  # ✅ Light green for kit rows
            )

            self.tree.tag_configure(
                "module_data", background="#C9E2FA"  # Light blue for module rows
            )
    ...
    def _build_with_headers(self, rows):
        ...
        for it in ordered:
            kit_code = (
                it.get("Kit") if it.get("Kit") and it.get("Kit") != "-----" else None
            )
            module_code = (
                it.get("module")
                if it.get("module") and it.get("module") != "-----"
                else None
            )
            ...
            if (
                module_code
                and module_number
                and (kit_code, module_code, module_number, kit_number)
                not in seen_modules
            ):
                result.append(
                    {
        ...
        for iid in self.tree.get_children():
            ...
            if rtype == "kit":
                ...
                qty = (
                    1
                    if (
                        "module" in {t.lower() for t in rules["editable_types"]}
                        and stock > 0
                    )
                    else 0
                )
    ...
    def _reapply_editable_icons(self, rules):
        ...
        for iid in self.tree.get_children():
            ...
            if (
                not meta.get("is_header")
                and rtype in editable_lower
                and meta.get("unique_id")
            ):
                core = vals[8]
    ...
    def on_Kit_selected(self, event=None):
        ...
        if kit_numbers:
            ...
            self.status_var.set(
                lang.t(
                    "out_kit.no_kit_numbers", "No kit numbers available with stock > 0"
                )
            )
    ...
    def on_Kit_number_selected(self, event=None):
        ...
        logging.debug(
            f"[OUT_KIT] Kit_number selected: {Kit_number}, Kit={Kit_code}, mode={mode_key}"
        )
        ...
        if mode_key == "out_kit":
            ...
            if modules:
                ...
                self.status_var.set(
                    lang.t(
                        "out_kit.select_module_msg",
                        "Select a module from dropdown ({count} available)",
                        count=len(modules),
                    )
                )
                logging.debug(
                    f"[OUT_KIT] Populated {len(modules)} modules for kit {Kit_code}"
                )
            else:
            ...
            module_numbers = self.fetch_module_numbers(
                self.selected_scenario_id, kit_number=Kit_number
            )
            ...
            if module_numbers:
                ...
                self.status_var.set(
                    lang.t(
                        "out_kit.select_module_number_msg",
    ...
    def on_module_selected(self, event=None):
        ...
        logging.debug(
            f"[OUT_KIT] Module selected: '{module_display}' -> Code: '{module_code}'"
        )
        ...
        module_numbers = self.fetch_module_numbers(
            self.selected_scenario_id, module_code=module_code, kit_number=Kit_number
        )
    ...
    def on_module_number_selected(self, event=None):
        ...
        logging.debug(
            f"[OUT_KIT] Module_number selected: {module_number}, module={module_code}, Kit={Kit_code}"
        )
    ...
    def populate_standalone_items(self):
        ...
        self.populate_rows(
            self.full_items, f"Loaded {len(self.full_items)} standalone rows"
        )
    ...
    def populate_rows(self, items=None, status_msg=""):
        ...
        logging.debug(
            f"[POPULATE_ROWS] Inserting {len(rows_with_headers)} rows ({len(items)} items)"
        )
        ...
        for item in rows_with_headers:
            ...
            if not is_header and item.get("unique_id"):
                is_adopted, original_comment = self.check_if_adopted_expiry(
                    item["unique_id"]
                )
            ...
            if is_header:
                ...
                if is_adopted:
                    ...
                    logging.debug(
                        f"[POPULATE] Missing required expiry: {item.get('code')} (exp: '{item.get('expiry_date')}')"
                    )
        ...
        if adopted_count > 0:
            ...
            msg = status_msg or lang.t(
                "out_kit.loaded_rows",
                "Loaded {count} rows",
                count=len(rows_with_headers),
            )

        self.status_var.set(msg)
        logging.info(
            f"[POPULATE_ROWS] Populated {len(rows_with_headers)} rows with highlights (adopted={adopted_count})"
        )
    ...
    def start_edit(self, event):
        ...
        if col_index == 6:
            ...
            if item_type == "ITEM":
                ...
                self.status_var.set(
                    lang.t(
                        "out_kit.expiry_only_items", "Expiry dates are only for items"
                    )
                )
            return
    ...
    def edit_qty_to_issue_popup(self, row_id, max_stock):
        ...
        def save_quantity():
            ...
            if not new_qty_str.isdigit():
                status_label.config(
                    text=lang.t(
                        "out_kit.error_invalid_number", "Please enter a valid number"
                    )
                )
                return
        ...
        self.status_var.set(
            lang.t("out_kit.qty_updated", "Quantity updated for {code}: {qty}").format(
                code=code, qty=new_qty
            )
        )

        logging.debug(
            f"[OUT_KIT] Updated {code} qty_to_issue: {current_qty} ��� {new_qty}"
        )
    ...
    def _start_edit_cell(self, row_id, col_index):
            ...
            def save_expiry(_=None):
                ...
                if not raw_input:
                    ...
                    if row_id in self.row_data:
                        ...
                        requires_expiry = False
                        if rtype == "item":
                            requires_expiry, _ = self.check_if_item_requires_expiry(
                                item_code
                            )
                        ...
                        if requires_expiry:
                            ...
                            tags.append("missing_required_expiry")
                            logging.warning(
                                f"[EDIT_EXPIRY] User cleared REQUIRED expiry for {item_code}"
                            )
                        else:
                            # Optional expiry cleared → cyan background
                            tags.append("expiry_edited")
                            logging.debug(
                                f"[EDIT_EXPIRY] User cleared optional expiry for {item_code}"
                            )
                    ...
                    self.status_var.set(
                        lang.t("out_kit.expiry_cleared", "✓ Expiry cleared")
                    )
                    cleanup()
                    return
                ...
                if row_id in self.row_data:
                    ...
                    logging.info(
                        f"[EDIT_EXPIRY] User edited: {meta.get('code')}: {old_expiry} → {parsed_date}"
                    )

                    self.status_var.set(
                        lang.t(
                            "out_kit.expiry_updated",
        ...
        # ===== QUANTITY EDITING (Column 8) =====
            ...
            raw_old = vals[8]
            old_clean = (
                raw_old[2:].strip() if raw_old.startswith("★") else raw_old.strip()
            )
            ...
            entry.bind(
                "<Escape>",
                lambda e: (
                    entry.destroy() if entry.winfo_exists() else None,
                    setattr(self, "editing_cell", None),
                ),
            )
            return
    ...
    def search_items(self, event=None):
        ...
        if q == "":
            self.populate_rows(
                self.full_items, f"Showing {len(self.full_items)} rows (reset)"
            )
            return
        if len(q) < self.search_min_chars:
            self.status_var.set(
                f"Type at least {self.search_min_chars} chars to search..."
            )
            return
        filtered = []
        for it in self.full_items:
            if (
                q in (it["code"] or "").lower()
    ...
    def _insert_transaction_out(
        ...
        *,
        ...
        try:
            ...
            logging.debug(
                f"[OUT_TRANSACTION] Logged OUT: {code} qty={qty_out} kit={kit} module={module}"
            )
        except sqlite3.Error as e:
            logging.error(f"[OUT_TRANSACTION] Failed: {e}")
            raise

    def _insert_transaction_in_mirror(
        self,
        cur,
        *,
        unique_id,
        ...
        try:
            ...
            if item_type_normalized in ("KIT", "MODULE"):
                actual_qty_in = 0
                logging.debug(
                    f"[MIRROR_IN] {item_type} being broken → qty_in=0 (releasing children)"
                )
            else:
    ...
    def _show_adopted_expiry_warning(self):
        ...
        # ✅ Show warning dialog
        logging.info(
            f"[SAVE] Found {adopted_count} items with adopted expiries, showing warning"
        )
        ...
        tk.Label(
            title_frame,
            text=lang.t(
                "out_kit.adopted_expiry_warning_title", "Adopted Expiry Dates Detected"
            ),
            font=("Helvetica", 14, "bold"),
            bg="#FFF3E0",
            fg="#E65100",
        ...
        logging.info(
            f"[SAVE] User chose: {'PROCEED' if result['proceed'] else 'REVIEW'}"
        )
    ...
    def _validate_expiry_dates_required(self):
        ...
        for iid in self.tree.get_children():
            ...
            if requires_expiry:
                ...
                if not is_valid_date:
                    ...
                    logging.warning(
                        f"[VALIDATE_EXPIRY] Missing/invalid expiry: {item_code} (exp: '{expiry_date}')"
                    )
        ...
        if is_valid:
            logging.info(
                "[VALIDATE_EXPIRY] ✓ All items requiring expiry have valid dates"
            )
        else:
            logging.warning(
                f"[VALIDATE_EXPIRY] ✗ {len(missing_items)} items missing valid expiry dates"
            )
    ...
    def _show_missing_expiry_error(self, missing_items):
        ...
        tk.Label(
            title_frame, text="❌", font=("Helvetica", 32), bg="#FFEBEE", fg="#C62828"
        ).pack(side="left", padx=(0, 10))
    ...
    def save_all(self):
        ...
        if not is_valid:
            ...
            self.status_var.set(
                lang.t(
                    "out_kit.save_blocked_missing_expiry",
                    "Save blocked - {count} items require expiry dates",
                    count=len(missing_items),
                )
            )
            logging.error(
                f"[SAVE] Blocked - {len(missing_items)} items missing required expiry dates"
            )
            return
        ...
        for iid in self.tree.get_children():
            ...
            if len(vals) < 12:
                logging.warning(
                    f"[OUT_KIT] Row {iid} has only {len(vals)} columns, skipping"
                )
                continue
        ...
        if not rows:
            custom_popup(
                ...
                lang.t("dialog_titles.error", "Error"),
                lang.t(
                    "out_kit.no_items_to_process", "No quantities entered to process."
                ),
                "error",
            )
            return
        ...
        self.status_var.set(
            lang.t(
                "out_kit.processing",
                "Processing... Document Number: {doc}",
                doc=doc_number,
            )
        )
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if conn is None:
                custom_popup(
                    ...
                    lang.t("dialog_titles.error", "Error"),
                    lang.t(
                        "out_kit.db_connection_failed", "Database connection failed."
                    ),
                    "error",
                )
                return
            ...
            try:
                ...
                for r in rows:
                    ...
                    out_comment = (
                        f"OUT from in-box {r['kit_number']}/{r['module_number']}"
                    )
                    if r["exp_date_adopted"]:
                        out_comment += " | Adopted_Expiration/Caducidad"
                    ...
                    logging.info(
                        f"[OUT_KIT] OUT processed: {r['code']} qty={r['qty_out']} from in-box"
                    )
                    ...
                    if r["qty_in"] > 0:
                        ...
                        if onshelf_existing:
                            ...
                            logging.info(
                                f"[OUT_KIT] Updated existing on-shelf: {r['unique_id_onshelf']}"
                            )
                            ...
                            if r["exp_date_adopted"] and not r["expiry_edited"]:
                                ...
                                logging.warning(
                                    f"[OUT_KIT] Preserved adopted expiry (not edited): {r['code']} = {r['exp_date']}"
                                )
                                ...
                                logging.info(
                                    f"[OUT_KIT] User verified expiry: {r['code']} = {r['exp_date']}"
                                )
                            ...
                            logging.info(
                                f"[OUT_KIT] Created on-shelf: {r['unique_id_onshelf']} (adopted={r['exp_date_adopted']}, edited={r['expiry_edited']})"
                            )
                        ...
                        logging.info(
                            f"[OUT_KIT] IN processed: {r['code']} qty={r['qty_in']} to on-shelf"
                        )
                ...
                if (
                    custom_askyesno(
                        ...
                        lang.t(
                            "out_kit.ask_export", "Export the break operation to Excel?"
                        ),
                    )
                    == "yes"
                ):
                ...
                if "locked" in str(e).lower() and attempt < max_attempts:
                    logging.warning(
                        f"[OUT_KIT] Database locked attempt {attempt}/{max_attempts}, retrying..."
                    )
                    try:
                        conn.rollback()
                    except:
                ...
                custom_popup(
                    ...
                    lang.t("dialog_titles.error", "Error"),
                    lang.t(
                        "out_kit.break_failed", "Break failed: {error}", error=str(e)
                    ),
                    "error",
                )
                return
        ...
        custom_popup(
            ...
            lang.t("dialog_titles.error", "Error"),
            lang.t(
                "out_kit.break_failed_locked", "Break failed: database remained locked."
            ),
            "error",
        )
    ...
    def export_data(self, rows_processed=None):
        ...
        try:
                ...
                export_rows.append(
                    {
                        ...
                        "current_stock": (
                            int(current_stock) if str(current_stock).isdigit() else 0
                        ),
                        "expiry_date": exp_date or "",
            ...
            if not any(r["qty_out"] > 0 for r in export_rows):
                custom_popup(
                    self.parent, "Error", "No break quantities to export.", "error"
                )
                return
            ...
            for row_idx, r in enumerate(export_rows, start=8):
                ...
                for col in range(1, len(headers) + 1):
                    ...
                    cell.font = Font(
                        name="Calibri", size=11, bold=(rtype in ("kit", "module"))
                    )
                    if rtype == "kit":
