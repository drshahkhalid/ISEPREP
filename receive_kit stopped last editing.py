...
class StockData:
    @staticmethod
    def add_or_update(unique_id, scenario, qty_in=0, qty_out=0, exp_date=None,
        ...
        try:
            ...
            if existing:
                ...
                if has_scenario:
                    cur.execute("""
                        INSERT INTO stock_data (unique_id, qty_in, qty_out, exp_date,
                                                kit_number, module_number, scenario, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """, (unique_id, qty_in, qty_out, exp_date, kit_number, module_number, scenario, now))
                else:
                    cur.execute("""
                        INSERT INTO stock_data (unique_id, qty_in, qty_out, exp_date,
                                                kit_number, module_number, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?)
...
class StockReceiveKit(tk.Frame):
    def __init__(self, parent, app, role: str = "supervisor"):
        ...
        self._cascade_module_qty = True  # Set False if you want module displayed qty NOT multiplied by kit count.
        # Data structures
    ...
    def on_kit_number_selected(self, event=None):
        ...
        for item in sorted(items, key=lambda x: x['treecode']):
            ...
            self._init_user_qty(iid)

        self._compute_multiplicative_quantities()
        self.recompute_exp_groups()
    ...
    def on_module_number_selected(self, event=None):
        ...
        for item in sorted(items, key=lambda x: x['treecode']):
            ...
            self._init_user_qty(iid)  # ADDED: ensure base qty captured
        self._compute_multiplicative_quantities()
        self.recompute_exp_groups()
    ...
    def load_hierarchy(self, code):
        ...
        for comp in sorted(comps, key=lambda x: x['treecode']):
            ...
            # Initialize base user quantity
            self._init_user_qty(iid)

        # Recompute multiplicative displayed quantities
        self._compute_multiplicative_quantities()
        self.recompute_exp_groups()
        ...
        def save_and_close(event=None):
            ...
            try:
                ...
                if col_index == 6:
                    ...
                    if new == "":
                        # Allow blank -> treat as 1 (choose 0 if you prefer)
                        new_qty = 1
                    else:
                        if not new.isdigit():
                        ...
                        if new_qty < 0:
                            ...
                            return
                    self._init_user_qty(row_id)
                    self.row_data[row_id]['user_qty'] = new_qty
                    was_structural = self._is_structural_row(row_id)
                    self._recalc_after_quantity_edit(row_id, was_structural)
                    edited = True
    ...
    def log_transaction(self, unique_id, code, description, expiry_date, batch_number,
    ...
    # ================== MULTIPLICATIVE QTY SYSTEM (ADDED) ==================
    def _init_user_qty(self, iid):
        """
        Ensure the row has a stored 'user_qty' (base entered quantity).
        If missing, derive from current displayed qty_to_receive (col index 6) or default to 1.
        """
        if iid not in self.row_data:
            self.row_data[iid] = {}
        rd = self.row_data[iid]
        if 'user_qty' not in rd:
            vals = self.tree.item(iid, "values")
            base = 1
    ...
    def _is_structural_row(self, iid):
        if not self.tree.exists(iid):
            return False
        vals = self.tree.item(iid, "values")
        if not vals: return False
        t = (vals[2] or "").strip().upper()
        return t in ("KIT","PRIMARY","MODULE","SECONDARY")
    ...
    def _compute_multiplicative_quantities(self):
        """
        Recompute displayed qty_to_receive using stored base user_qty values and
        structural (kit/module) multipliers.

        Returns:
            int: number of rows whose displayed quantity (column 6) changed.
        """
        if not self.tree:
            return 0

        # Maps for structural user quantities
        kit_qty = {}
        mod_qty = {}

        # Ensure user_qty is initialized
        all_iids = self._gather_full_tree_nodes()
        for iid in all_iids:
            self._init_user_qty(iid)

        # Collect structural base quantities
        for iid in all_iids:
            rd = self.row_data.get(iid, {})
            tc = rd.get('treecode')
            stype = self._tc_len_type(tc)
            if stype == "KIT":
                kit_qty[tc] = rd.get('user_qty', 1)
            elif stype == "MODULE":
                mod_qty[tc] = rd.get('user_qty', 1)

        changed = 0
        # Apply multiplicative logic
        for iid in all_iids:
            if not self.tree.exists(iid):
                continue
            vals = list(self.tree.item(iid, "values"))
            ...
            if stype == "KIT":
                ...
                if self._cascade_module_qty and tc and len(tc) >= 8:
                    kit_prefix = tc[:5]
                    kit_val = kit_qty.get(kit_prefix, 1)
                    display = base * kit_val
            elif stype == "ITEM":
                if tc and len(tc) == 11:
                    kit_prefix = tc[:5]
                    module_prefix = tc[:8]
                    kit_val = kit_qty.get(kit_prefix, 1)
                    mod_val = mod_qty.get(module_prefix, 1)
                    display = base * kit_val * mod_val

            if str(vals[6]) != str(display):
                vals[6] = str(display)
                self.tree.item(iid, values=tuple(vals))
                changed += 1
    ...
    def _recalc_after_quantity_edit(self, edited_iid, was_structural):
        ...
        If structural (KIT/MODULE), recompute entire tree (affects descendants).
        If item, still recompute all for simplicity (consistent parent multiplier handling).
        """
        if not self.tree.exists(edited_iid):
            return
        changed = self._compute_multiplicative_quantities()
        # changed is an int (>=0); no comparisons against None anymore
        if was_structural:
            if changed > 1:
    ...
    def update_row_comment(self, iid, force=False, sticky_mode=True):
        """
        Per-row comment updater (sticky by default).
        Logic (sticky_mode=True):
          - Structural rows (KIT / MODULE / PRIMARY / SECONDARY): comment always blank.
          - ITEM rows:
              * If user_manual_expiry (rd['user_manual_expiry']) -> blank; clear sticky_comment.
              * Else if no expiry display -> blank.
              * Else if already have rd['sticky_comment'] and not force:
                    keep it (do NOT recompute) unless equality conditions are now impossible
                    (e.g. exp_module & exp_kit both blank OR row expiry blank).
              * Else (first time or force):
                    If expiry == exp_module -> set "Adopted Module expiry"
                    Else if expiry == exp_kit -> set "Adopted Kit expiry"
                    Else blank.
        If sticky_mode=False it re-derives dynamically every call (not recommended for your stated need).

        This ONLY updates the specified row.
        """
        ...
        expiry_disp = (vals[7] or "").strip()
        exp_mod_disp = (vals[9] or "").strip()
        exp_kit_disp = (vals[10] or "").strip()
        rd = self.row_data.setdefault(iid, {})
        manual = bool(rd.get("user_manual_expiry"))

        # Structural rows always blank
        ...
        # Non-structural (ITEM or other)
        if manual or not expiry_disp:
            # Manual entry or no expiry wipes comment
            if vals[11] != "":
                vals[11] = ""
                rd['sticky_comment'] = ""
                self.tree.item(iid, values=tuple(vals))
        ...
        if sticky_mode:
            # If we have a sticky_comment already and not forcing, keep it unless no longer valid context
            if not force and 'sticky_comment' in rd:
                existing = rd['sticky_comment']
                if existing:
                    # Validate minimal context; if both group displays gone, clear.
                    if not exp_mod_disp and not exp_kit_disp:
                        rd['sticky_comment'] = ""
                        if vals[11] != "":
                            vals[11] = ""
                            self.tree.item(iid, values=tuple(vals))
                    ...
                    return
                # If existing is blank we may attempt adoption below (first time).
            # Derive adoption first time or forced
            if exp_mod_disp and expiry_disp == exp_mod_disp:
                new_comment = "Adopted Module expiry"
            elif exp_kit_disp and expiry_disp == exp_kit_disp:
    ...
    def recompute_exp_groups(self):
        """
        Recompute group earliest expiries with MODULE STRUCTURAL PRECEDENCE and
        treecode lengths: KIT=5, MODULE=8, ITEM=11.

        Rules:
          - Structural module expiry overrides descendant earliest (even if later or earlier).
          - If module structural expiry absent:
              * Prefer earliest item expiry under the module that is DIFFERENT from the kit structural expiry (if kit struct exists),
                so a true module-specific earlier/later date is not masked by inherited kit values.
              * If none differ, fall back to earliest item (which may equal kit).
          - Kit earliest: structural kit expiry if present, else earliest item expiry under that kit.
          - Items then auto-fill / upgrade via _auto_fill_expiry_with_precedence (module precedence).
        """
        if not self.tree or not self.selected_scenario_id:
            return

        # Data stores
        kit_struct = {}        # kit_prefix (len 5) -> structural kit expiry
        module_struct = {}     # module_prefix (len 8) -> structural module expiry
        kit_item_expiries = {}     # kit_prefix -> list[item_iso]
        module_item_expiries = {}  # module_prefix -> list[item_iso]

        # Pass 1: walk rows, collect structural and item expiries
        for iid in self._gather_full_tree_nodes():
            if not self.tree.exists(iid):
                continue
            vals = self.tree.item(iid, "values")
            ...
            # Ensure rd["expiry_iso"] is populated if cell has a parsable date
            if not rd.get("expiry_iso"):
                raw_cell = (vals[7] or "").strip()
                if raw_cell:
                    iso = parse_expiry(raw_cell)
                    if iso:
                        rd["expiry_iso"] = iso

            iso = rd.get("expiry_iso")

            # Structural detection by treecode length
            L = len(tc)
            if L == 5 and rtype in ("KIT", "PRIMARY"):
                if iso:
                    kit_struct[tc[:5]] = iso
            elif L == 8 and rtype in ("MODULE", "SECONDARY"):
                if iso:
                    module_struct[tc[:8]] = iso
            elif L == 11 and rtype not in ("KIT", "PRIMARY", "MODULE", "SECONDARY"):
                # Treat as ITEM
                if iso:
                    kpref = tc[:5]
                    mpref = tc[:8]
                    kit_item_expiries.setdefault(kpref, []).append(iso)
                    module_item_expiries.setdefault(mpref, []).append(iso)

        # Pass 2: determine earliest kit expiry
        earliest_kit = {}
        for kpref, struct_iso in kit_struct.items():
            earliest_kit[kpref] = struct_iso  # structural precedence
        # Fill missing kit entries from items if no structural
        for kpref, items in kit_item_expiries.items():
            if kpref not in earliest_kit and items:
                earliest_kit[kpref] = min(items)

        # Pass 3: determine earliest module expiry with structural precedence & difference preference
        earliest_module = {}
        for mpref, struct_iso in module_struct.items():
            earliest_module[mpref] = struct_iso  # structural precedence

        for mpref, items in module_item_expiries.items():
            if not items:
                continue
            if mpref in earliest_module:
                continue  # already have structural
            kpref = mpref[:5]
            kit_struct_iso = kit_struct.get(kpref)
            # If kit structural exists, look for item expiry that differs (module-specific)
            if kit_struct_iso:
                differing = [d for d in items if d != kit_struct_iso]
                if differing:
                    earliest_module[mpref] = min(differing)
                else:
                    earliest_module[mpref] = min(items)  # fallback (will likely equal kit)
            else:
                # No kit structural, just earliest item
                earliest_module[mpref] = min(items)

        # Pass 4: write exp_module / exp_kit columns & store iso in row_data
        for iid in self._gather_full_tree_nodes():
            if not self.tree.exists(iid):
                continue
            vals = list(self.tree.item(iid, "values"))
            ...
            if tc:
                L = len(tc)
                # Module context: any node under a module prefix len>=8
                if L >= 8 and tc[:8] in earliest_module:
                    rd["exp_module_iso"] = earliest_module[tc[:8]]
                    exp_mod_disp = format_expiry_display(rd["exp_module_iso"])
                # Kit context: any node under a kit prefix len>=5
                if L >= 5 and tc[:5] in earliest_kit:
        ...
        # Pass 5: auto fill / upgrade precedence
        self._auto_fill_expiry_with_precedence()

        # Pass 6: Retag + comments
        for iid in self._gather_full_tree_nodes():
    ...
    def _auto_fill_expiry_with_precedence(self):
        ...
        for iid in self._gather_full_tree_nodes():
            ...
            if exp_mod:
                ...
                # No module earliest; adopt kit earliest only if none set
                if current is None and exp_kit:
                    rd["expiry_iso"] = exp_kit
                    changed = True
    ...
    def _rederive_comments(self, force=False, sticky_mode=True):
        """
        Bulk wrapper calling update_row_comment for every row.
        force=True will re-evaluate even existing sticky comments.
        """
        if not self.tree:
            return
        for iid in self._gather_full_tree_nodes():
    ...
    def add_new_row(self):
        ...
        self.row_data[new_iid] = {
            ...
            "user_qty": 1  # explicit base
        }
        self._init_user_qty(new_iid)
        self._compute_multiplicative_quantities()
        self.update_unique_ids_and_column()
        ...
        try:
            ...
            if type_field.upper() == "KIT":
                while not kit_number:
                    entered = simpledialog.askstring("Kit Number", f"Enter Kit Number for {code}", parent=self.parent)
                    if entered is None:
                        return False
            ...
            if type_field.upper() == "MODULE":
                while not module_number:
                    entered = simpledialog.askstring("Module Number", f"Enter Module Number for {code}", parent=self.parent)
                    if entered is None:
                        return False
            ...
            if qty_to_receive > 0:
                ...
                self.log_transaction(
                    unique_id, code, desc, final_expiry, batch_no or None,
                    scenario_name, kit_number, module_number, qty_to_receive,
                    self.trans_type_var.get() or "stock_in",
                    None, None,
                    self.third_party_var.get() or None,
                    self.end_user_var.get() or None,
                    self.remarks_entry.get().strip() or None,
                    self.mode_var.get() or "stock_in",
                    document_number
                )
                exported_rows.append({
                    'code': code, 'description': desc, 'type': type_field, 'kit': kit_val,
                    'module': mod_val, 'std_qty': std_qty, 'qty_to_receive': qty_to_receive,
                    'expiry_date': final_expiry or '', 'batch_no': batch_no,
                    'exp_module': exp_module_col, 'exp_kit': exp_kit_col, 'comments': comments_col,
                    'unique_id': unique_id
                })
            for child in self.tree.get_children(iid):
                ok = self.save_subtree(
                    child, invalid_items, exported_rows,
                    kit_number, module_number, document_number,
